...
from models.modules import PointModule, PointSequential, PDNorm
...
class SerializedAttention(PointModule):
    ...
    def forward(self, point):
        ...
        if not self.enable_flash:
            #TODO remove this code, changed because torch moreeffective with F.scaled_dot_product_attention
            ...
            # qkv: (N', K, 3, H, C')
            q, k, v = qkv.reshape(-1, K, 3, H, C // H).unbind(dim=2)
            ...
            attn_bias = None
            if self.enable_rpe:
                attn_bias = self.rpe(self.get_rel_pos(point, order))  # (N', H, K, K)
...
@MODELS.register_module("PT-v3-Base")
