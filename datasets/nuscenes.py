...
from .utils import points2image
    ...
    def get_data(self, idx):
        ...
        if self.with_images:
            ...
            for cam in data["cams"].values():
                ...
                # Per-point camera-frame depth is returned for the aux-depth head
                # (model K). It's already computed during projection; we just keep it.
                points_image, visibility_mask, points_depth = points2image(
                    coord, points2cam, K, images[-1].shape[:2], return_depth=True
                )
                image_coords.append(points_image)
                image_masks.append(visibility_mask)
                image_depths.append(points_depth)
            ...
            data_dict["image_depth"] = np.stack(image_depths, axis=1).astype(
                np.float32
            )  # (N, CAM) camera-frame depth; invisible entries unused (masked in loss)
